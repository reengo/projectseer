...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    // get ID
    if (isset($smarty->_tpl_vars['paginate_id'])) {
        $_id = $smarty->_tpl_vars['paginate_id'];
    }
    ...
    while($_item <= $_total) {
        ...
        if(isset($_page_limit) && $_display_pages == $_page_limit) {
            if ($_total_page != $_page - 1) {
                $_ret .= ' ... ';
                $_item = ($_total % $_limit > 0) ? $_total - ( $_total % $_limit ) + 1 : $_total - $_limit + 1;
                $_page = $_total_page;
            }
            $_last_page = true;
        }
