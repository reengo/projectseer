...
class Admin_UatController extends ApplicationController 
{
    public function indexAction()
    {           
    	 $this->_redirect('/admin/uat/list');
    }
                ...
                if($params['id'])
                {
                	...
                	if($project !== null){
                		...
                		if($project->save()){                			
                    		...
                    		$this->_redirect('/admin/uat/view/id/'.$params['id']);
                		}
                	}
                }     
            }
    ...
    public function deleteAction()
    {
       ...
       if ($reviewData->save()) {
            ...
            $this->_redirect('admin/uat/list/id/'.$reviewInfo['form_id']);
        }         
	}
            ...
            if ($this->validator->add->isValid()) {
        		...
        		if($this->reviews->insert($data)){   
        			if($data['layout'] == 2 || $data['rollover'] == 2 || $data['animation'] == 2 || $data['forms'] == 2 || $data['link'] == 2 || $data['source'] == 2 || $data['actions'] == 2 || $data['validation'] == 2){
        				...
        				$this->_redirect('/admin/uat/add-ticket/id/'. $qaInfo['id']);
        			}          			
        			$this->flash('Review <b>' . $data['page'] . '</b> has been successfully added.');
            		$this->_redirect('/admin/uat/list/id/'.$data['form_id']);
        ...
        if ($this->form->isSubmitted()) {
            ...
            if ($this->validator->add_ticket->isValid()) {
                ...
                if($this->tickets->insert($ticketData)){   
                	...
                	$uatReview = $this->reviews->find($params['id'])->current();  
                	              	
            		$uatReview->setFromArray($updateRating);
            		$uatReview->save();
        			...
        			$qaProfile = $this->user_profile->fetchRow(array('user_id = ?' => $projectInfo['uat_officer']))->toArray();
		            ...
		            $this->_redirect('/admin/uat/add-ticket/id/'.$params['id']);          		
        		}            	             
            }
        }
    }
